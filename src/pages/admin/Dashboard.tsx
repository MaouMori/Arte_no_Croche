...
export default function Dashboard() {
  ...
  const stats = {
    ...
    avgTicket: orders.length > 0
      ? orders.filter(o => o.status === 'concluido').reduce((sum, o) => sum + o.total, 0) / orders.length
      : 0,
  }
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="review-card rounded-xl p-5">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link to="/admin/produtos" className="text-xs text-neon-pink hover:text-hot-pink transition-colors">Ver todos</Link>
          </div>
          <div className="space-y-3">
            {recentProducts.length === 0 && (
              <p className="text-text-dim text-sm text-center py-4">Nenhum produto cadastrado.</p>
            )}
            {recentProducts.map(product => (
              <div key={product.id} className="flex items-center gap-3 p-2 rounded-lg hover:bg-void-lighter/50 transition-colors">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-neon-pink">R$ {product.price.toFixed(2).replace('.', ',')}</p>
                </div>
                <span className="text-[10px] bg-green-500/10 text-green-400 px-2 py-0.5 rounded-full">Ativo</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="review-card rounded-xl p-5">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link to="/admin/pedidos" className="text-xs text-neon-pink hover:text-hot-pink transition-colors">Ver todos</Link>
          </div>
          <div className="space-y-3">
            {recentOrders.length === 0 && (
              <p className="text-text-dim text-sm text-center py-4">Nenhum pedido ainda.</p>
            )}
            {recentOrders.map(order => (
              <div key={order.id} className="flex items-center gap-3 p-2 rounded-lg hover:bg-void-lighter/50 transition-colors">
                <div className="w-10 h-10 rounded-full bg-void-lighter overflow-hidden flex-shrink-0">
                  <img src={order.customerAvatar} alt={order.customer} className="w-full h-full object-cover"
                    onError={(e) => { (e.target as HTMLImageElement).style.display = 'none' }}
                  />
                </div>
                <div className="flex-1 min-w-0">
                ...
                <span className={`text-[10px] px-2 py-0.5 rounded-full ${
                  order.status === 'concluido' ? 'bg-green-500/10 text-green-400' :
                  order.status === 'em_processamento' ? 'bg-yellow-500/10 text-yellow-400' :
                  order.status === 'pago' ? 'bg-blue-500/10 text-blue-400' :
                  'bg-red-500/10 text-red-400'
                }`}>
                  {order.status === 'concluido' ? 'Concluido' :
                   order.status === 'em_processamento' ? 'Em processamento' :
                   order.status === 'pago' ? 'Pago' : 'Cancelado'}
                </span>
                <p className="text-sm font-semibold text-text-main">R$ {order.total.toFixed(2).replace('.', ',')}</p>
              </div>
            ))}
          </div>
        </div>
          ...
          <div className="flex items-end gap-2 h-40 mb-4">
            {salesData.map((d, i) => (
              <div key={i} className="flex-1 flex flex-col items-center gap-1">
                <div className="w-full bg-neon-pink/20 rounded-t-lg relative group" style={{ height: `${(d.value / maxSales) * 100}%` }}>
                  <div className="absolute -top-6 left-1/2 -translate-x-1/2 bg-void-lighter border border-neon-pink/20 rounded px-1.5 py-0.5 text-[10px] text-text-main opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap">R$ {d.value}</div>
                </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="review-card rounded-xl p-5">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link to="/admin/cargos" className="text-xs bg-neon-pink hover:bg-hot-pink text-white px-3 py-1.5 rounded-lg transition-colors flex items-center gap-1"><Plus className="w-3 h-3" />Novo cargo</Link>
          </div>
          <div className="space-y-2">
            {roles.map(role => (
              <div key={role.id} className="flex items-center gap-3 p-3 rounded-lg bg-void-lighter/30">
                <span className="text-xs font-bold px-2 py-1 rounded" style={{ backgroundColor: role.color + '20', color: role.color }}>{role.color}</span>
                <span className="text-sm text-text-main flex-1">{role.name}</span>
                <span className="text-[10px] text-text-dim">{role.permissions.join(', ')}</span>
                <div className="flex gap-1">
                  <Link to="/admin/cargos" className="p-1.5 text-text-dim hover:text-neon-pink transition-colors"><Pencil className="w-3.5 h-3.5" /></Link>
                  <Link to="/admin/cargos" className="p-1.5 text-text-dim hover:text-red-400 transition-colors"><Trash2 className="w-3.5 h-3.5" /></Link>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="review-card rounded-xl p-5 lg:col-span-2">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link to="/admin/produtos" className="text-xs bg-neon-pink hover:bg-hot-pink text-white px-3 py-1.5 rounded-lg transition-colors flex items-center gap-1"><Plus className="w-3 h-3" />Novo produto</Link>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full">
              ...
              <tbody className="text-sm">
                {products.length === 0 && (
                  <tr><td colSpan={6} className="py-8 text-center text-text-dim">Nenhum produto cadastrado.</td></tr>
                )}
                {products.slice(0, 5).map(product => (
                  <tr key={product.id} className="border-t border-neon-pink/5 hover:bg-void-lighter/30 transition-colors">
                    ...
                    <td className="py-3">
                      <div className="flex gap-1">
                        <Link to="/admin/produtos" className="p-1.5 text-text-dim hover:text-neon-pink transition-colors"><Pencil className="w-3.5 h-3.5" /></Link>
                        ...
                        <button className="p-1.5 text-text-dim hover:text-red-400 transition-colors"><Trash2 className="w-3.5 h-3.5" /></button>
                      </div>
                    </td>
                  </tr>
                ))}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="review-card rounded-xl p-5">
          ...
          <div className="grid grid-cols-2 gap-2">
            {[
              ...
              <Link key={i} to={action.link} className="flex items-center gap-2 p-3 rounded-lg bg-void-lighter/30 hover:bg-void-lighter transition-colors text-left">
        ...
        <div className="review-card rounded-xl p-5 lg:col-span-2">
          ...
          <div className="space-y-3">
            {activities.length === 0 && (
              <p className="text-text-dim text-sm text-center py-4">Nenhuma atividade registrada.</p>
            )}
